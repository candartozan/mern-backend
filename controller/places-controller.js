...
const getPlaceById = async (req, res, next) => {
    ...
    if (!place) {
        const error = new HttpErrorError(
...
const updatePlace = async (req, res, next) => {
    ...
    if (place.creator.toString() !== req.userData.userId) {
        const error = new HttpError(
            "You are nor allowed to edit this place.",
            401
        );
        return next(error);
    }
...
const deletePlace = async (req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError(
            "Something went wrong, could not delete any places.",
            500
        );
        return next(error);
    }
    ...
    if (place.creator.id !== req.userData.userId) {
        const error = new HttpError(
            "You are nor allowed to delete this place.",
            401
        );
        return next(error);
    }
